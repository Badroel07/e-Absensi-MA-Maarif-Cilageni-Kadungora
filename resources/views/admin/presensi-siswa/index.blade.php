...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
            <h1 class="text-2xl font-black text-slate-900 heading-font tracking-tight">Presensi Siswa</h1>
        ...
        <a href="{{ route('admin.audit.index') }}"
           class="inline-flex items-center gap-2 text-xs font-bold text-slate-700 bg-white hover:bg-slate-50 active:bg-slate-100 border border-slate-200/80 px-3.5 py-2 rounded-xl transition-all duration-150 active:scale-95 shadow-2xs">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-maarif-800 to-maarif-900 rounded-2xl p-5 text-white flex flex-col justify-between">
            <div>
                <p class="text-[11px] font-extrabold text-emerald-300 uppercase tracking-widest">Siswa Hadir</p>
                <div class="mt-2.5 flex items-baseline gap-2">
                    <span class="text-4xl font-black mono-font tracking-tight leading-none">{{ $hadirTotal }}</span>
                    <span class="text-xs font-bold text-emerald-200/80">/ {{ $totalPresensi }} Siswa</span>
                </div>
            </div>
            <div class="mt-4">
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            ...
            <div class="mt-3">
                <span class="text-3xl font-black text-amber-600 mono-font">{{ $summary['izin'] }}</span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            ...
            <div class="mt-3">
                <span class="text-3xl font-black text-sky-600 mono-font">{{ $summary['sakit'] }}</span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            ...
            <div class="mt-3">
                <span class="text-3xl font-black text-rose-600 mono-font">{{ $summary['alpa'] }}</span>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 p-4">
        <form method="GET" action="{{ route('admin.presensi-siswa.index') }}" class="flex flex-wrap items-center gap-3">
            <div class="w-40">
                    ...
                    @forelse($attendances as $att)
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            ...
                            <td class="py-3.5 px-5">
                                <p class="font-bold text-slate-900 text-sm leading-snug">{{ $att->student->name ?? '-' }}</p>
                            ...
                            <td class="py-3.5 px-5">
                                <div class="flex flex-wrap items-center gap-1.5">
                                    ...
                                    <span class="font-bold text-slate-800">{{ $att->schedule->subject->name ?? '-' }}</span>
                                </div>
                            </td>
                            ...
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                @if($sessionLocked)
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-bold bg-slate-100 text-slate-600 border border-slate-200" title="Sesi ditutup & disimpan oleh guru">
                                        <i data-lucide="lock" class="w-3 h-3 text-slate-500"></i>
                                        <span>Tersimpan</span>
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-extrabold bg-emerald-50 text-emerald-800 border border-emerald-200/70">
                        ...
                        <tr>
                            <td colspan="7" class="py-16 text-center">
                                <div class="inline-flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-bold text-slate-600">Tidak ada data kehadiran siswa yang ditemukan</p>
        ...
        <div class="bg-white rounded-3xl max-w-lg w-full shadow-2xl border border-slate-200/80 max-h-[92vh] flex flex-col overflow-hidden transform transition-all">
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/60 shrink-0">
                <div class="flex items-center gap-3.5">
                    ...
                    <div>
                        <h3 id="modalKoreksiSiswaTitle" class="text-base sm:text-lg font-extrabold text-slate-900 heading-font">Koreksi Presensi Siswa</h3>
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-5 text-xs">
                ...
                <div class="bg-slate-50/80 rounded-2xl p-4 border border-slate-200/80 space-y-2">
                    <div class="flex justify-between items-center">
                        ...
                        <span id="modalStudentName" class="font-extrabold text-slate-900 text-sm">-</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-slate-500 font-medium">NISN:</span>
                        <span id="modalNisn" class="mono-font font-bold text-slate-700 px-2 py-0.5 rounded-md bg-white border border-slate-200/70">-</span>
                    </div>
                    <div class="flex justify-between items-center">
                    ...
                    <div class="flex justify-between items-center pt-2 border-t border-slate-200/70">
                        ...
                        <span id="modalCurrentStatus" class="font-extrabold px-2.5 py-0.5 rounded-lg text-xs bg-slate-200/80 text-slate-800">-</span>
                    </div>
                </div>
                ...
                <form id="correctionForm" method="POST" action="" class="space-y-4">
                    ...
                    <div>
                        <label class="block font-bold text-slate-700 mb-2 uppercase tracking-wider text-[11px]">Pilih Status Baru <span class="text-rose-500">*</span></label>
                        <div class="grid grid-cols-4 gap-2">
                            <label class="cursor-pointer">
                                <input type="radio" name="status" value="HADIR" id="radioHadir" class="peer hidden">
                                <span class="block py-2.5 px-1 text-center font-extrabold rounded-xl border border-slate-200/90 bg-white text-slate-700 peer-checked:bg-maarif-700 peer-checked:text-white peer-checked:border-maarif-700 shadow-xs transition hover:bg-slate-50">
                                    Hadir
                                </span>
                            </label>
                            <label class="cursor-pointer">
                                <input type="radio" name="status" value="IZIN" id="radioIzin" class="peer hidden">
                                <span class="block py-2.5 px-1 text-center font-extrabold rounded-xl border border-slate-200/90 bg-white text-slate-700 peer-checked:bg-amber-600 peer-checked:text-white peer-checked:border-amber-600 shadow-xs transition hover:bg-slate-50">
                                    Izin
                                </span>
                            </label>
                            <label class="cursor-pointer">
                                <input type="radio" name="status" value="SAKIT" id="radioSakit" class="peer hidden">
                                <span class="block py-2.5 px-1 text-center font-extrabold rounded-xl border border-slate-200/90 bg-white text-slate-700 peer-checked:bg-sky-600 peer-checked:text-white peer-checked:border-sky-600 shadow-xs transition hover:bg-slate-50">
                                    Sakit
                                </span>
                            </label>
                            <label class="cursor-pointer">
                                <input type="radio" name="status" value="ALPA" id="radioAlpa" class="peer hidden">
                                <span class="block py-2.5 px-1 text-center font-extrabold rounded-xl border border-slate-200/90 bg-white text-slate-700 peer-checked:bg-rose-600 peer-checked:text-white peer-checked:border-rose-600 shadow-xs transition hover:bg-slate-50">
                                    Alpa
                                </span>
                            </label>
                        </div>
                    ...
                    <div>
                        <label for="modalReason" class="block font-bold text-slate-700 mb-1.5 uppercase tracking-wider text-[11px]">
