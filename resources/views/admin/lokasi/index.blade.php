...
<div class="max-w-3xl space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 heading-font tracking-tight">Pengaturan Lokasi Madrasah</h1>
    ...
    <div class="bg-white rounded-2xl p-6 sm:p-7 border border-slate-200/80 space-y-6">
        <div class="flex items-start gap-3.5 pb-4 border-b border-slate-100">
            ...
            <div>
                <h2 class="font-bold text-slate-900 heading-font text-base">Titik Koordinat Resmi Madrasah</h2>
        ...
        <form action="{{ route('admin.lokasi.update') }}" method="POST" data-loading-form class="space-y-5 text-xs">
            @csrf

            <div>
                <label class="block font-semibold text-slate-700 mb-1.5">Nama Lokasi Madrasah <span class="text-rose-500">*</span></label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block font-semibold text-slate-700 mb-1.5">Titik Lintang (Latitude) <span class="text-rose-500">*</span></label>
                    <input type="number" step="0.0000001" name="latitude" value="{{ old('latitude', $location->latitude ?? -7.1147000) }}" required
                        class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-maarif-600 focus:border-maarif-600 focus:outline-none bg-slate-50 focus:bg-white mono-font font-medium text-xs transition">
                </div>

                <div>
                    <label class="block font-semibold text-slate-700 mb-1.5">Titik Bujur (Longitude) <span class="text-rose-500">*</span></label>
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="number" name="radius_meters" min="30" max="500" value="{{ old('radius_meters', $location->radius_meters ?? 75) }}" required
                        class="w-32 px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-maarif-600 focus:border-maarif-600 focus:outline-none bg-slate-50 focus:bg-white mono-font text-center font-semibold text-sm transition">
            ...
            <div class="p-4 rounded-xl bg-maarif-50 border border-maarif-200/70 text-slate-700 text-xs space-y-1">
                <p class="font-semibold text-maarif-800 flex items-center gap-1.5">
