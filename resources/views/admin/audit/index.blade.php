...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
            <h1 class="text-2xl font-black text-slate-900 heading-font tracking-tight">Riwayat Perubahan Data</h1>
            <p class="text-xs text-slate-500 mt-0.5">Catatan audit log resmi perubahan status presensi dan penyesuaian oleh operator.</p>
        </div>
        <span class="text-xs font-bold px-3 py-1.5 bg-white text-slate-700 rounded-xl mono-font border border-slate-200/80 shadow-2xs self-start sm:self-auto">
                    ...
                    @forelse($auditLogs as $log)
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            ...
                            <td class="py-3.5 px-5 whitespace-nowrap">
                                <span class="font-mono text-slate-700 text-xs font-bold block">{{ $log->created_at->format('d/m/Y') }}</span>
                                <span class="font-mono text-slate-400 text-[11px]">{{ $log->created_at->format('H:i:s') }} WIB</span>
                            </td>

                            {{-- Siswa --}}
                            <td class="py-3.5 px-5">
                                <p class="font-bold text-slate-900 text-sm leading-snug">{{ $log->lessonAttendance->student->name ?? '-' }}</p>
                            ...
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                <div class="inline-flex items-center gap-1.5">
                                    <span class="px-2 py-0.5 rounded-md text-[11px] font-extrabold border {{ $badgeOld }}">
                                        {{ $log->old_status }}
                                    </span>
                                    <i data-lucide="arrow-right" class="w-3.5 h-3.5 text-slate-400"></i>
                                    <span class="px-2 py-0.5 rounded-md text-[11px] font-extrabold border {{ $badgeNew }}">
                                        {{ $log->new_status }}
                                    </span>
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colspan="5" class="py-16 text-center">
                                <div class="inline-flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-bold text-slate-600">Belum ada riwayat perubahan presensi</p>
