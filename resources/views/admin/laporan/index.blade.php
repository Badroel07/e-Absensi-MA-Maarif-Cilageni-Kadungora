...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
            <h1 class="text-2xl font-black text-slate-900 heading-font tracking-tight">Laporan Kehadiran Siswa</h1>
        ...
        <div class="flex items-center gap-2">
            <button type="button" onclick="openPdfModal()"
                class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 active:bg-rose-800 text-white font-extrabold text-xs shadow-sm transition-all duration-150 active:scale-95 cursor-pointer">
                <i data-lucide="printer" class="w-4 h-4"></i>
                <span>Cetak PDF</span>
            </button>

            <a href="{{ route('admin.laporan.excel', request()->all()) }}"
                class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-maarif-700 hover:bg-maarif-800 active:bg-maarif-900 text-white font-extrabold text-xs shadow-sm transition-all duration-150 active:scale-95 cursor-pointer">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 p-4">
        <form method="GET" action="{{ route('admin.laporan.index') }}" class="flex flex-wrap items-center gap-3 text-xs">
            <div class="flex items-center gap-2">
                <div>
                    <label class="block font-bold text-slate-700 mb-1 text-[11px]">Mulai</label>
                    <input type="date" name="start_date" value="{{ $startDate }}"
                        class="px-3 py-2 text-xs border border-slate-200 rounded-xl focus:ring-2 focus:ring-maarif-600 focus:outline-none bg-slate-50 focus:bg-white mono-font font-bold transition">
                </div>
                <div>
                    <label class="block font-bold text-slate-700 mb-1 text-[11px]">Sampai</label>
            ...
            <div>
                <label class="block font-bold text-slate-700 mb-1 text-[11px]">Kelas</label>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="bg-gradient-to-br from-maarif-800 to-maarif-900 rounded-2xl p-5 text-white flex flex-col justify-between">
            <div>
                <p class="text-[11px] font-extrabold text-emerald-300 uppercase tracking-widest">Rata-rata Kehadiran</p>
                <div class="mt-2 flex items-baseline gap-1">
                    <span class="text-4xl font-black mono-font tracking-tight leading-none">{{ $stats['percentage_hadir'] }}%</span>
                </div>
            </div>
            <div class="mt-4">
                ...
                <p class="text-[11px] text-emerald-200/80 font-semibold mt-1.5">Presensi hadir terverifikasi</p>
            </div>
        </div>

        {{-- Hadir --}}
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            <p class="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest">Hadir (JP)</p>
            <div class="mt-2">
                <span class="text-3xl font-black text-emerald-700 mono-font">{{ number_format($stats['total_hadir'], 0, ',', '.') }}</span>
        ...
        {{-- Izin --}}
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            <p class="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest">Izin (JP)</p>
            <div class="mt-2">
                <span class="text-3xl font-black text-amber-600 mono-font">{{ number_format($stats['total_izin'], 0, ',', '.') }}</span>
        ...
        {{-- Sakit --}}
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            <p class="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest">Sakit (JP)</p>
            <div class="mt-2">
                <span class="text-3xl font-black text-sky-600 mono-font">{{ number_format($stats['total_sakit'], 0, ',', '.') }}</span>
        ...
        {{-- Alpa --}}
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            <p class="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest">Alpa (JP)</p>
            <div class="mt-2">
                <span class="text-3xl font-black text-rose-600 mono-font">{{ number_format($stats['total_alpa'], 0, ',', '.') }}</span>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h2 class="font-extrabold text-slate-900 heading-font text-sm leading-tight">Rekapitulasi Kehadiran Siswa</h2>
                <p class="text-[11px] text-slate-500 mt-0.5">Daftar agregat kehadiran seluruh siswa per kelas terpilih.</p>
            </div>
            <span class="text-xs font-bold text-slate-700 bg-slate-100 px-2.5 py-1 rounded-md border border-slate-200 mono-font">
                {{ count($rows) }} Siswa
            </span>
        </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($rows as $idx => $r)
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            ...
                            <td class="py-3 px-5 font-mono font-bold text-slate-700">{{ $r['nisn'] }}</td>
                            <td class="py-3 px-5 font-bold text-slate-900 text-sm">{{ $r['name'] }}</td>
                            <td class="py-3 px-5">
                                <span class="px-2 py-0.5 rounded-md bg-maarif-50 text-maarif-800 border border-maarif-200/70 font-extrabold text-[11px]">
                                    Kelas {{ $r['class_name'] }}
                                </span>
                            </td>
                            <td class="py-3 px-4 text-center font-mono font-bold text-emerald-700">{{ $r['hadir'] }}</td>
                            <td class="py-3 px-4 text-center font-mono font-bold text-amber-600">{{ $r['izin'] }}</td>
                            <td class="py-3 px-4 text-center font-mono font-bold text-sky-600">{{ $r['sakit'] }}</td>
                            <td class="py-3 px-4 text-center font-mono font-bold text-rose-600">{{ $r['alpa'] }}</td>
                            <td class="py-3 px-5 text-center font-mono text-slate-600 font-semibold">{{ $r['total'] }}</td>
                            <td class="py-3 px-5 text-right font-mono font-black text-slate-900 whitespace-nowrap">
                                <span class="px-2 py-0.5 rounded-md text-[11px] font-extrabold {{ $r['persentase'] >= 80 ? 'bg-emerald-50 text-emerald-800 border border-emerald-200/70' : 'bg-rose-50 text-rose-800 border border-rose-200/70' }}">
                                    {{ $r['persentase'] }}%
                                </span>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td colspan="10" class="py-16 text-center">
                                <div class="inline-flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-bold text-slate-600">Tidak ada data presensi pada rentang tanggal ini</p>
    ...
    <div class="bg-white rounded-3xl max-w-2xl w-full max-h-[92vh] flex flex-col overflow-hidden shadow-2xl border border-slate-200/80 transform transition-all">
        ...
        <div class="flex items-center justify-between px-6 py-5 border-b border-slate-100 bg-slate-50/60 shrink-0">
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    <h3 id="modalPdfTitle" class="text-base sm:text-lg font-extrabold text-slate-900 heading-font">Lembar Pengesahan Laporan PDF</h3>
        ...
        <form id="pdfExportForm" method="GET" action="{{ route('admin.laporan.pdf') }}" target="_blank" class="flex-1 overflow-y-auto p-6 space-y-5 text-xs">
            ...
            <div class="space-y-3.5 bg-slate-50/80 p-4 rounded-2xl border border-slate-200/80">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <span class="font-extrabold text-slate-900 uppercase tracking-wider text-[11px] flex items-center gap-2">
                    ...
                    <div class="flex flex-wrap gap-1 text-[10.5px]">
                        <button type="button" onclick="setSignerPreset('Waka. Kesiswaan', 'Ahmad Subandi, S.AP', '198501012010011001')" class="px-2.5 py-1 rounded-lg bg-white border border-slate-200 hover:bg-slate-100 font-bold text-slate-700 cursor-pointer shadow-xs transition">Waka Kesiswaan</button>
                        <button type="button" onclick="setSignerPreset('Kepala Tata Usaha', 'Dra. Hj. Siti Aminah', '197905122008012003')" class="px-2.5 py-1 rounded-lg bg-white border border-slate-200 hover:bg-slate-100 font-bold text-slate-700 cursor-pointer shadow-xs transition">Kepala TU</button>
                        <button type="button" onclick="setSignerPreset('Waka. Kurikulum', 'Drs. H. M. Zainuri, M.Pd', '197508142000031002')" class="px-2.5 py-1 rounded-lg bg-white border border-slate-200 hover:bg-slate-100 font-bold text-slate-700 cursor-pointer shadow-xs transition">Waka Kurikulum</button>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                    <label class="block font-bold text-slate-700 mb-1">Kota Pengesahan</label>
                ...
                <div>
                    ...
                    <input type="date" name="signature_date" value="{{ \Carbon\Carbon::today()->format('Y-m-d') }}"
                        class="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-maarif-600 focus:outline-none bg-slate-50/70 focus:bg-white mono-font font-bold text-xs">
                </div>
            </div>

            <div class="flex items-center justify-end gap-2.5 pt-4 border-t border-slate-100">
                <button type="button" onclick="closePdfModal()"
                    class="py-2.5 px-5 rounded-xl bg-slate-100 hover:bg-slate-200/80 text-slate-700 font-bold text-xs border border-slate-200 cursor-pointer transition-all active:scale-[0.98]">
                    Batal
                </button>
                <button type="submit" onclick="setTimeout(closePdfModal, 500)"
                    class="py-2.5 px-6 rounded-xl bg-rose-600 hover:bg-rose-700 active:bg-rose-800 text-white font-extrabold text-xs shadow-xs flex items-center gap-2 cursor-pointer transition-all active:scale-[0.98]">
                    <i data-lucide="printer" class="w-4 h-4"></i>
