...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 heading-font tracking-tight">Riwayat Presensi Siswa</h1>
    ...
    <div class="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            @if($student->profile_photo_url)
                <img src="{{ $student->profile_photo_url }}" loading="lazy" decoding="async" alt="{{ $student->name }}"
            ...
            <div>
                <div class="flex items-center gap-2.5">
                    <h2 class="text-lg font-bold text-slate-900 heading-font leading-tight">{{ $student->name }}</h2>
                ...
                <div class="flex flex-wrap items-center gap-2 mt-1 text-xs text-slate-500 font-medium">
                    ...
                    <span>Kelas: <strong class="text-maarif-800 font-semibold">Kelas {{ $student->classroom->name ?? '-' }}</strong></span>
                    <span class="text-slate-300">•</span>
        ...
        <div class="flex items-center gap-2 self-start md:self-auto">
            <span class="px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 font-semibold text-xs">
                Email: <span class="mono-font text-slate-900 font-semibold">{{ $student->email ?? '-' }}</span>
            </span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-maarif-800 to-maarif-900 rounded-2xl p-5 text-white flex flex-col justify-between">
            <div>
                ...
                <div class="mt-2.5 flex items-baseline gap-2">
                    ...
                    <span class="text-xs font-semibold text-emerald-200/80">({{ $totalHadir }} Hadir)</span>
                </div>
            </div>
            <div class="mt-4">
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            ...
            <div class="mt-3">
                <span class="text-3xl font-bold text-amber-600 mono-font">{{ $totalIzin }}</span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            ...
            <div class="mt-3">
                <span class="text-3xl font-bold text-sky-600 mono-font">{{ $totalSakit }}</span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            ...
            <div class="mt-3">
                <span class="text-3xl font-bold text-rose-600 mono-font">{{ $totalAlpa }}</span>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 p-4">
        <form method="GET" action="{{ route('admin.siswa.riwayat', $student) }}" data-loading-form class="flex flex-wrap items-center gap-3 text-xs">
            <div>
                <label class="block font-semibold text-slate-700 mb-1 text-[11px]">Dari Tanggal</label>
                <input type="date" name="start_date" value="{{ $startDate }}"
                    class="px-3.5 py-2 text-xs border border-slate-200 rounded-xl focus:ring-2 focus:ring-maarif-600 focus:outline-none bg-slate-50 focus:bg-white mono-font font-semibold transition">
            </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
                <h3 class="font-bold text-slate-900 heading-font text-sm">Rekam Jejak Presensi Mata Pelajaran</h3>
                    ...
                    @forelse($attendances as $att)
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            ...
                            <td class="py-3.5 px-5 whitespace-nowrap">
                                <p class="font-semibold text-slate-900">{{ $att->attendance_date->format('d M Y') }}</p>
                            ...
                            <td class="py-3.5 px-5">
                                <span class="font-semibold text-slate-900 text-sm block">{{ $att->schedule->subject->name ?? 'Mata Pelajaran' }}</span>
                                @if($att->schedule)
                            ...
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                ...
                                @elseif($att->status === 'SAKIT')
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-semibold bg-sky-50 text-sky-800 border border-sky-200">
                                        <i data-lucide="activity" class="w-3 h-3 text-sky-600"></i>
                                        Sakit
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-semibold bg-rose-50 text-rose-800 border border-rose-200/70">
                        ...
                        <tr>
                            <td colspan="5" class="py-16 text-center">
                                <div class="inline-flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-semibold text-slate-600">Belum ada riwayat presensi pada rentang tanggal ini</p>
            ...
            @forelse($attendances as $att)
                ...
                <div class="p-4 rounded-2xl border border-slate-200/80 bg-white {{ $borderClass }} shadow-xs space-y-2.5">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        @if($att->status === 'HADIR')
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200/80 shrink-0">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                Hadir
                            </span>
                        @elseif($att->status === 'IZIN')
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-amber-50 text-amber-800 border border-amber-200/80 shrink-0">
                                <i data-lucide="file-text" class="w-3 h-3 text-amber-600"></i>
                                Izin
                            </span>
                        @elseif($att->status === 'SAKIT')
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-sky-50 text-sky-800 border border-sky-200/80 shrink-0">
                                <i data-lucide="activity" class="w-3 h-3 text-sky-600"></i>
                                Sakit
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-rose-50 text-rose-800 border border-rose-200/80 shrink-0">
                                <i data-lucide="alert-circle" class="w-3 h-3 text-rose-600"></i>
                                Alpa
                            </span>
                        @endif
                    ...
                    <div class="pt-1">
                        <h4 class="font-semibold text-slate-900 text-sm heading-font">
                    ...
                    <div class="pt-2 border-t border-slate-100 flex flex-col gap-1 text-[11px]">
                        <div class="flex items-center justify-between text-slate-500">
                            ...
                            <span class="font-semibold text-slate-800 mono-font">
                                {{ $att->verified_at ? $att->verified_at->format('H:i:s').' WIB' : 'Oleh Guru' }}
                            </span>
                        </div>
                        @if($att->notes)
