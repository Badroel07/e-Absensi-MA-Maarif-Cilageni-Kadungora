...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Catatan kedatangan (scan QR) dan absen per sesi mengajar guru.</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.presensi-guru.index') }}"
               class="inline-flex items-center gap-1.5 px-3.5 py-2 bg-maarif-700 hover:bg-maarif-800 active:bg-maarif-900 text-white font-semibold rounded-xl text-xs transition-all duration-150 active:scale-95 shadow-sm">
                <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i>
                <span>Kembali ke Kehadiran</span>
            </a>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 p-4">
        <form method="GET" action="{{ route('admin.presensi-guru.riwayat', $teacher) }}" data-loading-form class="flex flex-wrap items-center gap-3 text-xs">
            <div>
            ...
            <div class="self-end pt-1 flex items-center gap-2">
                ...
                @if(request()->hasAny(['start_date', 'end_date']))
                    <a href="{{ route('admin.presensi-guru.riwayat', $teacher) }}"
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
                <h3 class="font-bold text-slate-900 heading-font text-sm">Rekam Jejak Kedatangan Guru</h3>
                <p class="text-[11px] text-slate-500 mt-0.5">Catatan presensi masuk via scan QR di Layar Presensi Madrasah.</p>
            </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($attendances as $att)
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            ...
                            {{-- Jam Pulang dihapus: absen guru per sesi mengajar --}}
                            {{-- Jarak GPS --}}
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
                <h3 class="font-bold text-slate-900 heading-font text-sm">Rekam Jejak Absen Sesi Mengajar</h3>
                <p class="text-[11px] text-slate-500 mt-0.5">Absen otomatis tercatat saat guru membuka Sesi Presensi pada jadwalnya.</p>
            </div>
            <span class="text-xs font-semibold text-slate-700 bg-slate-100 px-3 py-1 rounded-md border border-slate-200 mono-font self-start sm:self-auto">
                {{ $sessionAttendances->count() }} sesi tercatat
            </span>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100 text-slate-500 uppercase font-semibold text-[10px] tracking-widest">
                        <th class="py-3 px-5">Hari &amp; Tanggal</th>
                        <th class="py-3 px-5">Mapel / Kelas</th>
                        <th class="py-3 px-5 text-center">Waktu Mapel</th>
                        <th class="py-3 px-5 text-center">Jam Absen</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($sessionAttendances as $sa)
                        @php
                            $saLabels = ['HADIR' => 'Hadir', 'IZIN' => 'Izin', 'SAKIT' => 'Sakit', 'DINAS_LUAR' => 'Dinas Luar', 'ALPA' => 'Alpa'];
                        @endphp
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            <td class="py-3.5 px-5">
                                <p class="font-semibold text-slate-900">{{ $sa->attendance_date->translatedFormat('l, d F Y') }}</p>
                            </td>
                            <td class="py-3.5 px-5">
                                <p class="font-semibold text-slate-900">{{ $sa->schedule->subject->name ?? '-' }}</p>
                                <p class="text-[11px] text-slate-400 mt-0.5">Kelas {{ $sa->schedule->classroom->name ?? '-' }}</p>
                            </td>
                            <td class="py-3.5 px-5 text-center font-mono whitespace-nowrap">
                                {{ substr($sa->schedule->start_time, 0, 5) }} – {{ substr($sa->schedule->end_time, 0, 5) }}
                            </td>
                            <td class="py-3.5 px-5 text-center font-mono whitespace-nowrap">
                                @if($sa->attended_at)
                                    <span class="inline-block font-semibold text-slate-900 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200 text-xs">
                                        {{ $sa->attended_at->format('H:i') }} WIB
                                    </span>
                                @else
                                    <span class="text-slate-300 font-sans">—</span>
                                @endif
                            ...
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                @if($sa->status === 'HADIR')
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-semibold {{ $sa->isLate() ? 'bg-amber-50 text-amber-800 border border-amber-200' : 'bg-emerald-50 text-emerald-800 border border-emerald-200/70' }}">
                                        <span class="w-1.5 h-1.5 rounded-full {{ $sa->isLate() ? 'bg-amber-500' : 'bg-emerald-500' }} shrink-0"></span>
                                        Hadir{{ $sa->isLate() ? ' (Terlambat)' : '' }}
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-semibold bg-rose-50 text-rose-800 border border-rose-200/70">
                                        {{ $saLabels[$sa->status] ?? $sa->status }}
                                    </span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="py-16 text-center">
                                <div class="inline-flex flex-col items-center gap-2">
                                    <span class="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center">
                                        <i data-lucide="presentation-x" class="w-6 h-6 text-slate-400"></i>
                                    </span>
                                    <p class="text-sm font-semibold text-slate-600">Belum ada absen sesi mengajar pada rentang tanggal ini</p>
                                    <p class="text-xs text-slate-400">Coba sesuaikan tanggal awal dan akhir filter di atas.</p>
                                </div>
                            </td>
                        </tr>
