...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
            <h1 class="text-2xl font-black text-slate-900 heading-font tracking-tight">Riwayat Presensi Guru</h1>
    ...
    <div class="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/80 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            <div>
                <div class="flex items-center gap-2.5">
                    <h2 class="text-lg font-black text-slate-900 heading-font leading-tight">{{ $teacher->name }}</h2>
        ...
        <div class="flex items-center gap-2 self-start md:self-auto">
            <span class="px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-600 font-semibold text-xs">
                Email: <span class="mono-font text-slate-900 font-bold">{{ $teacher->email ?? '-' }}</span>
            </span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-gradient-to-br from-maarif-800 to-maarif-900 rounded-2xl p-5 text-white flex flex-col justify-between">
            <div>
                ...
                <div class="mt-2.5 flex items-baseline gap-2">
                    ...
                    <span class="text-xs font-bold text-emerald-200/80">({{ $totalHadir }} Hari)</span>
                </div>
            </div>
            <div class="mt-4">
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            ...
            <div class="mt-3">
                <span class="text-3xl font-black text-emerald-700 mono-font">{{ $totalHadir }}</span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            ...
            <div class="mt-3">
                <span class="text-3xl font-black text-amber-600 mono-font">{{ $totalTerlambat }}</span>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
            <div class="flex items-center justify-between">
                <p class="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest">Dispensasi / Izin</p>
                <span class="w-8 h-8 rounded-xl bg-sky-50 border border-sky-200/70 text-sky-600 flex items-center justify-center">
                    <i data-lucide="file-text" class="w-4 h-4"></i>
                </span>
            </div>
            <div class="mt-3">
                <span class="text-3xl font-black text-sky-600 mono-font">{{ $totalIzinSakit }}</span>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 p-4">
        <form method="GET" action="{{ route('admin.guru.riwayat', $teacher) }}" class="flex flex-wrap items-center gap-3 text-xs">
            <div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
                <h3 class="font-extrabold text-slate-900 heading-font text-sm">Rekam Jejak Presensi Harian Guru</h3>
                    ...
                    @forelse($attendances as $att)
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            ...
                            <td class="py-3.5 px-5">
                                <p class="font-bold text-slate-900">{{ $att->attendance_date->translatedFormat('l, d F Y') }}</p>
                            </td>
                            ...
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-extrabold bg-slate-100 text-slate-600 border border-slate-200">
                                        {{ $att->check_in_status ?: '-' }}
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="py-3.5 px-5 text-center font-mono">
                                @if($att->check_in_time)
                                    <span class="inline-block font-bold text-slate-900 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200 text-xs">
                            ...
                            <td class="py-3.5 px-5 text-right font-mono whitespace-nowrap">
                                @if($att->check_in_distance_meters !== null)
                                    <span class="font-bold text-slate-800 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-200/70 text-xs">
                        ...
                        <tr>
                            <td colspan="5" class="py-16 text-center">
                                <div class="inline-flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-bold text-slate-600">Belum ada catatan presensi pada rentang tanggal ini</p>
