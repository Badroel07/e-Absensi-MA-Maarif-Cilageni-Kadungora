...
<div class="space-y-7">
    ...
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-1.5">
        <div>
            ...
            <h1 class="text-2xl font-bold text-slate-900 heading-font tracking-tight leading-tight">
                Dashboard Utama
            </h1>
        ...
        <div class="shrink-0 text-right">
            <p id="liveDashClock" class="text-xl font-bold mono-font text-slate-800 tracking-tight leading-none tabular-nums"></p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div class="lg:col-span-2 bg-gradient-to-br from-maarif-800 to-maarif-900 rounded-2xl p-6 flex flex-col justify-between text-white relative overflow-hidden">
            ...
            <div class="relative">
                <p class="text-xs font-semibold text-emerald-300 uppercase tracking-widest">Kehadiran Siswa Hari Ini</p>
                <div class="mt-3 flex items-baseline gap-2.5">
                    <span class="text-5xl font-bold mono-font tracking-tight leading-none">{{ number_format($siswaHadir, 0, ',', '.') }}</span>
                    <span class="text-sm font-semibold text-emerald-200/80">/ {{ number_format($totalSiswa, 0, ',', '.') }} siswa</span>
                </div>
                {{-- Progress bar --}}
                <div class="mt-4">
                    <div class="flex justify-between items-center mb-1.5">
                        <span class="text-xs font-semibold text-emerald-200">Tingkat Partisipasi</span>
                        <span class="text-xs font-bold mono-font text-white">{{ $persenSiswaHadir }}%</span>
                    </div>
            ...
            <div class="relative mt-5 flex items-center gap-4 text-xs font-semibold text-emerald-200/80">
                <span><span class="font-bold text-amber-300 text-sm">{{ number_format($siswaIzin, 0, ',', '.') }}</span> Izin</span>
                <span class="text-white/20">|</span>
                <span><span class="font-bold text-sky-300 text-sm">{{ number_format($siswaSakit, 0, ',', '.') }}</span> Sakit</span>
                <span class="text-white/20">|</span>
                <span><span class="font-bold text-rose-300 text-sm">{{ number_format($siswaAlpa, 0, ',', '.') }}</span> Alpa</span>
            </div>
        </div>
        ...
        <div class="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
                <div class="flex items-center justify-between mb-3">
                    <p class="text-[11px] font-semibold text-slate-500 uppercase tracking-widest">Guru Hadir</p>
                ...
                <div>
                    <div class="flex items-baseline gap-1.5">
                        <span class="text-3xl font-bold text-slate-900 mono-font">{{ number_format($guruHadir, 0, ',', '.') }}</span>
            ...
            <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
                <div class="flex items-center justify-between mb-3">
                    <p class="text-[11px] font-semibold text-slate-500 uppercase tracking-widest">Izin &amp; Sakit</p>
                ...
                <div>
                    <span class="text-3xl font-bold text-amber-700 mono-font">{{ number_format($siswaIzin + $siswaSakit, 0, ',', '.') }}</span>
                    <p class="text-[11px] text-slate-500 font-medium mt-2">
                        <span class="font-semibold text-amber-700">{{ number_format($siswaIzin, 0, ',', '.') }}</span> izin &bull;
                        <span class="font-semibold text-sky-700">{{ number_format($siswaSakit, 0, ',', '.') }}</span> sakit
                    </p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-5 border border-slate-200/80 flex flex-col justify-between">
                ...
                <div>
                    <span class="text-3xl font-bold text-rose-700 mono-font">{{ number_format($siswaAlpa, 0, ',', '.') }}</span>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden">
        ...
        <div class="p-5 border-b border-slate-100 flex items-center justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="font-bold text-slate-900 heading-font text-sm leading-tight">
                        Presensi Dewan Guru
            ...
            <a href="{{ route('admin.presensi-guru.index') }}"
               class="text-xs font-semibold text-white inline-flex items-center gap-1.5 shrink-0 bg-maarif-700 hover:bg-maarif-800 active:bg-maarif-900 px-3.5 py-2 rounded-xl transition-all duration-150 active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-maarif-600">
                <span>Kelola</span>
                    ...
                    @forelse($recentGuruAttendances as $gAtt)
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            <td class="py-3.5 px-5">
                                <p class="font-semibold text-slate-900 text-sm leading-snug">{{ $gAtt->user->name ?? 'Guru' }}</p>
                            ...
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                ...
                                @if($status === 'HADIR')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200/70">Tepat Waktu</span>
                                @elseif($status === 'TERLAMBAT')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-800 border border-amber-200">Terlambat</span>
                                @elseif($status === 'IZIN')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-800 border border-amber-200">Izin</span>
                                @elseif($status === 'SAKIT')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-sky-50 text-sky-800 border border-sky-200">Sakit</span>
                                @elseif($status === 'ALPA')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-rose-50 text-rose-800 border border-rose-200/70">Alpa</span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-100 text-slate-700 border border-slate-200">{{ $status }}</span>
                                @endif
                            </td>
                        </tr>
                    @empty
            ...
            <table class="w-full text-left border-collapse text-xs">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($recentSiswaAttendances as $sAtt)
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            <td class="py-3.5 px-5">
                                <p class="font-semibold text-slate-900 text-sm leading-snug">{{ $sAtt->student->name ?? 'Siswa' }}</p>
                                <p class="text-[11px] text-slate-400 mono-font mt-0.5">NISN: {{ $sAtt->student->identity_number ?? '-' }}</p>
                            </td>
                            <td class="py-3.5 px-5">
                                <div class="flex flex-wrap items-center gap-1.5">
                                    <span class="px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 text-[11px] font-semibold border border-slate-200">
                                        {{ $sAtt->student->classroom->name ?? ($sAtt->schedule->classroom->name ?? '-') }}
                                    </span>
                                    <span class="text-slate-700 font-medium text-xs">{{ $sAtt->schedule->subject->name ?? '-' }}</span>
                                </div>
                            </td>
                            <td class="py-3.5 px-5 text-center font-semibold mono-font text-slate-600 text-xs">
                                {{ ($sAtt->verified_at ?? $sAtt->created_at)?->format('H:i') ? ($sAtt->verified_at ?? $sAtt->created_at)->format('H:i') . ' WIB' : '—' }}
                            </td>
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                @if($sAtt->status === 'HADIR')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200/70">Hadir</span>
                                ...
                                @elseif($sAtt->status === 'ALPA')
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-rose-50 text-rose-800 border border-rose-200/70">Alpa</span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-slate-100 text-slate-700 border border-slate-200">{{ $sAtt->status }}</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="py-14 text-center">
                                <div class="inline-flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-semibold text-slate-600">Belum ada presensi siswa hari ini</p>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex items-center gap-3">
            ...
            <div>
                <h3 class="font-bold text-slate-900 heading-font text-sm leading-tight">Sesi Kelas Berlangsung</h3>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($activeSessions as $ses)
                        <tr class="hover:bg-slate-50/60 transition-colors duration-100">
                            <td class="py-3.5 px-5">
                                <div class="flex flex-wrap items-center gap-1.5">
                                    <span class="px-2 py-0.5 rounded-md bg-maarif-50 text-maarif-800 text-[11px] font-semibold border border-maarif-200/70">
                                        Kelas {{ $ses->schedule->classroom->name }}
                                    </span>
                                    <span class="font-semibold text-slate-900 text-sm">{{ $ses->schedule->subject->name }}</span>
                                </div>
                            </td>
                            <td class="py-3.5 px-5 text-slate-700 font-medium">{{ $ses->teacher->name }}</td>
                            <td class="py-3.5 px-5 text-center font-bold mono-font text-maarif-700 text-base tracking-widest">{{ $ses->pin_code }}</td>
                            ...
                            <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-rose-50 text-rose-800 border border-rose-200/70">Waktu Habis</span>
                                @endif
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="py-14 text-center">
                                <div class="inline-flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-semibold text-slate-600">Tidak ada sesi kelas aktif saat ini</p>
