...
<style>
    ...
    @keyframes scanLineMove { 0%{transform:translateY(0);opacity:0.8} 50%{transform:translateY(180px);opacity:0.3} 100%{transform:translateY(0);opacity:0.8} }
...
@section('content')
<div class="max-w-md mx-auto sm:max-w-xl space-y-5">

    <!-- PageHeaderIntro (header hijau dibiarkan di layout) -->
    <section class="space-y-1.5" data-purpose="header-intro">
        <h2 class="text-xl font-extrabold text-slate-900 tracking-tight heading-font">Pemindai QR Presensi</h2>
        <p class="text-xs text-slate-500 leading-relaxed">
            Arahkan kamera ke kode QR dinamis di Layar Presensi Madrasah
        </p>
        @if(isset($hasCheckedIn) && isset($dailyAttendance))
            <p class="text-xs font-medium text-emerald-700 pt-0.5 flex items-center gap-1.5">
    ...
    @if(isset($dailyAttendance) && $dailyAttendance?->check_out_time)
        <div class="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-sm text-center space-y-5">
    ...
    @else
        <!-- BEGIN: ScannerSection -->
        <section class="bg-white rounded-3xl p-4 shadow-sm border border-slate-200/70 space-y-4" data-purpose="qr-scanner-card">
            ...
            <!-- Viewfinder Viewport -->
            <div class="relative w-full rounded-2xl overflow-hidden bg-slate-900 shadow-inner flex items-center justify-center" style="aspect-ratio: 4/3; min-height: 260px;">
                ...
                <div class="relative w-52 h-44 z-10 flex items-center justify-center pointer-events-none">
                    ...
                    <div id="scanReticle" class="w-32 h-24 border border-white/60 rounded-lg flex items-center justify-center bg-white/5 backdrop-blur-[1px]">
                ...
                <div class="absolute bottom-2.5 inset-x-0 text-center z-10">
                    <span class="text-[11px] text-white/80 font-medium tracking-wide bg-black/40 px-3 py-1 rounded-full backdrop-blur-sm">Sejajarkan QR di dalam bingkai</span>
                </div>
                ...
                <div id="scannerPlaceholder" class="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-slate-950/95 p-6 text-center">
                    ...
                    <button type="button" onclick="startCamera()" class="mt-1 inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 text-white text-xs font-semibold shadow-md transition active:scale-[0.98] min-h-[44px]">
            ...
            <div class="flex items-center justify-between gap-3 pt-1">
                <div class="flex items-center gap-2 text-slate-500 pl-1">
                    <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect height="18" rx="2" ry="2" width="18" x="3" y="3"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
                    <span class="text-xs text-slate-600 font-medium">Tahan QR di tengah bingkai</span>
                </div>
                <button type="button" onclick="startCamera()" class="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-slate-700 bg-slate-50 hover:bg-slate-100 active:scale-95 border border-slate-200 rounded-xl transition-all shadow-2xs">
                    <svg class="w-3.5 h-3.5 text-slate-600" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M21 12a9 9 0 0 0-9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path><path d="M3 12a9 9 0 0 0 9 9 9.75 9.75 0 0 0 6.74-2.74L21 16"></path><path d="M16 21h5v-5"></path></svg>
        ...
        <!-- BEGIN: LocationVerificationCard -->
        <section id="scanGeofenceCard" class="bg-white rounded-3xl p-4 shadow-sm border border-slate-200/70 space-y-3.5" data-purpose="location-card">
            <div class="flex items-start justify-between gap-3">
                <div class="flex items-start gap-3">
                    <div id="scanGeofenceIconBox" class="w-10 h-10 rounded-2xl bg-emerald-50 text-emerald-700 flex items-center justify-center shrink-0 border border-emerald-100">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 2a8 8 0 00-8 8c0 5.25 8 12 8 12s8-6.75 8-12a8 8 0 00-8-8z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                    </div>
                    <div class="space-y-0.5">
                        <h3 id="scanGeofenceTitle" class="text-sm font-bold text-slate-900 leading-tight heading-font">Di Lingkungan Madrasah</h3>
                        <p id="scanGeofenceDesc" class="text-xs text-slate-500 leading-relaxed">Radius {{ $location->radius_meters ?? 75 }} m — lokasi terverifikasi siap presensi.</p>
                    </div>
                </div>
                <div id="scanGeofenceDistance" class="text-xs font-bold text-emerald-700 bg-emerald-50/80 px-2.5 py-1 rounded-lg border border-emerald-200/60 shrink-0 mono-font heading-font">-- m</div>
            </div>
            <button type="button" onclick="initScanGeolocation(true)" class="w-full py-2.5 px-4 bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold rounded-xl border border-slate-200 flex items-center justify-center gap-2 active:scale-[0.99] transition-all shadow-2xs">
                <svg id="scanGpsRefreshIcon" class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 12a9 9 0 00-9-9 9.75 9.75 0 00-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path><path d="M3 12a9 9 0 009 9 9.75 9.75 0 006.74-2.74L21 16"></path><path d="M16 21h5v-5"></path></svg>
                <span>Perbarui Akurasi Lokasi</span>
            </button>
        </section>
        <!-- END: LocationVerificationCard -->

        <!-- BEGIN: ManualTokenAccordion -->
            ...
            <details class="group bg-white rounded-2xl shadow-sm border border-slate-200/70 overflow-hidden">
                <summary class="w-full p-4 flex items-center justify-between text-left cursor-pointer list-none">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center group-open:text-emerald-700 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 11-7.778 7.778 5.5 5.5 0 017.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"></path></svg>
                        </div>
                        <span class="text-xs font-semibold text-slate-800">Input Manual Token (Cadangan)</span>
                    </div>
                    <svg class="w-4 h-4 text-slate-400 group-open:rotate-180 transition-transform" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"></polyline></svg>
                </summary>
                <div class="px-4 pb-4 pt-2 border-t border-slate-100 space-y-3">
                    ...
                    <form id="formManual" onsubmit="submitManualToken(event)" class="flex gap-2">
                        ...
                        <button type="submit" id="btnSubmitManual" class="shrink-0 px-4 py-2.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white font-semibold text-xs shadow-sm min-h-[42px]">Kirim</button>
                    </form>
                </div>
            </details>
        </section>
        <!-- END: ManualTokenAccordion -->

        <!-- BEGIN: GuidanceNotesCard -->
        <section class="bg-slate-50/90 rounded-3xl p-4 border border-slate-200/60 space-y-2.5" data-purpose="attendance-info">
            <div class="flex items-center gap-2 text-slate-800">
                <svg class="w-4 h-4 text-emerald-700 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"></circle><line x1="12" x2="12" y1="16" y2="12"></line><line x1="12" x2="12.01" y1="8" y2="8"></line></svg>
                <h4 class="text-xs font-bold text-slate-800 heading-font">Informasi Presensi Guru</h4>
            </div>
            <ul class="text-[11px] text-slate-600 space-y-1.5 pl-6 list-disc leading-relaxed marker:text-slate-400">
        ...
        <div id="modalLock" class="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4 hidden">
            <div class="w-full max-w-sm bg-white rounded-3xl p-6 shadow-2xl space-y-4 border border-rose-200">
                ...
                <div class="flex items-center gap-2.5 pt-1">
                    <button type="button" onclick="closeLockModal()" class="flex-1 py-3 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs min-h-[44px]">Tutup</button>
...
(function() {
    ...
    function renderScanGeofence(data) {
        ...
        if (isWithinGeofence) {
            ...
            if (scanGeofenceTitle) { scanGeofenceTitle.textContent = "Di Lingkungan Madrasah"; scanGeofenceTitle.className = "text-sm font-bold text-slate-900 heading-font leading-tight"; }
            if (scanGeofenceDesc) scanGeofenceDesc.textContent = "Radius " + (data.radius || 75) + " m — lokasi terverifikasi siap presensi.";
            if (scanGeofenceDistance) { scanGeofenceDistance.textContent = Math.round(data.distance || 0) + " m"; scanGeofenceDistance.className = "text-xs font-bold text-emerald-700 bg-emerald-50/80 px-2.5 py-1 rounded-lg border border-emerald-200/60 shrink-0 mono-font heading-font"; }
        } else {
            ...
            if (scanGeofenceTitle) { scanGeofenceTitle.textContent = "Di Luar Area Madrasah"; scanGeofenceTitle.className = "text-sm font-bold text-rose-900 heading-font leading-tight"; }
            if (scanGeofenceDesc) scanGeofenceDesc.textContent = "Berada di luar " + (data.radius || 75) + " m dari madrasah. Presensi ditolak.";
            if (scanGeofenceDistance) { scanGeofenceDistance.textContent = Math.round(data.distance || 0) + " m"; scanGeofenceDistance.className = "text-xs font-bold text-rose-600 bg-rose-50/80 px-2.5 py-1 rounded-lg border border-rose-100 shrink-0 mono-font heading-font"; }
        }
    }
    ...
    function initScanGeolocation(isManual = false) {
        ...
        const handleScanError = (err) => {
            if (err.code === 1) {
                if (scanGeofenceTitle) { scanGeofenceTitle.textContent = "Izin lokasi ditolak"; scanGeofenceTitle.className = "text-sm font-bold text-rose-700 heading-font leading-tight"; }
                ...
                if (scanGeofenceTitle) { scanGeofenceTitle.textContent = "Mencari sinyal GPS..."; scanGeofenceTitle.className = "text-sm font-bold text-amber-700 heading-font leading-tight"; }
                if (scanGeofenceDesc) scanGeofenceDesc.textContent = "Aktifkan GPS & tunggu sinyal terkunci.";
                if (scanGeofenceDistance) scanGeofenceDistance.textContent = "-- m";
                scheduleScanRetry(3500);
            } else if (err.code === 3) {
                if (!userCoords.lat) {
                    if (scanGeofenceTitle) { scanGeofenceTitle.textContent = "Mencari lokasi GPS..."; scanGeofenceTitle.className = "text-sm font-bold text-amber-700 heading-font leading-tight"; }
