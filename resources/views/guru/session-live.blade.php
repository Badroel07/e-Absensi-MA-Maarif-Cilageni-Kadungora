...
<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white rounded-2xl p-4 border border-slate-200/80 shadow-xs">
        <div class="flex items-center justify-between sm:justify-start">
            <a href="{{ route('guru.dashboard') }}" class="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 active:bg-slate-300 text-slate-700 text-xs font-bold transition-all duration-150 active:scale-95 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-400">
        ...
        <div class="flex items-center gap-2.5 sm:gap-3 flex-wrap sm:justify-end">
            <span class="px-2.5 sm:px-3 py-1 rounded-lg text-xs font-extrabold bg-maarif-700 text-white shrink-0 shadow-xs">
                Kelas {{ $session->schedule->classroom->name }}
            </span>
            <span class="text-sm sm:text-base font-extrabold text-slate-900 heading-font truncate">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div class="lg:col-span-5 space-y-4">
            <div class="bg-white rounded-3xl p-5 sm:p-7 lg:p-8 border border-slate-200/80 shadow-md text-center space-y-5">
                <div class="inline-flex items-center space-x-2 px-3.5 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold animate-pulse">
                    <span class="w-2.5 h-2.5 rounded-full bg-emerald-600"></span>
                    <span>SESI PRESENSI KELAS AKTIF</span>
                </div>

                <div>
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-widest">Kode PIN Kelas (4 Angka)</p>
                    <div class="my-3 py-4 sm:py-6 px-3 sm:px-6 bg-slate-950 text-emerald-400 rounded-3xl mono-font font-black text-5xl sm:text-6xl lg:text-7xl tracking-widest sm:tracking-[0.25em] pl-[0.1em] sm:pl-[0.25em] shadow-inner flex items-center justify-center select-all border-2 border-slate-800">
                        {{ $session->pin_code }}
                    </div>
                ...
                <div class="p-4 bg-slate-50 rounded-2xl border border-slate-200/80">
                    <div class="flex items-center justify-between text-xs font-bold text-slate-700 mb-2">
                        ...
                        <span id="timerText" class="mono-font text-base sm:text-lg text-amber-600 font-black">--:--</span>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-3 sm:gap-4 pt-2 border-t border-slate-100 text-center">
                    <div class="p-3 sm:p-3.5 rounded-2xl bg-slate-50 border border-slate-100">
                        ...
                        <p class="text-xl sm:text-2xl font-black text-slate-800 mono-font mt-0.5">{{ $totalStudents }}</p>
                    </div>
                    <div class="p-3 sm:p-3.5 rounded-2xl bg-emerald-50 border border-emerald-100">
                        ...
                        <p id="verifiedCounter" class="text-xl sm:text-2xl font-black text-emerald-600 mono-font mt-0.5">{{ $verifiedCount }}</p>
                    </div>
                </div>

                <!-- Action Button -->
                <div class="pt-2">
                    <a href="{{ route('guru.session.reconcile', $session) }}" class="w-full bg-maarif-700 hover:bg-maarif-800 active:bg-maarif-900 active:scale-[0.98] text-white font-bold py-3.5 px-4 rounded-xl sm:rounded-2xl text-xs sm:text-sm inline-flex items-center justify-center gap-2 transition-all duration-150 shadow-lg shadow-maarif-700/25 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-maarif-600">
        ...
        <div class="lg:col-span-7 space-y-4">
            <div class="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200/80 shadow-xs flex flex-col h-full min-h-[400px] sm:min-h-[450px]">
                <div class="pb-3 border-b border-slate-100 mb-3">
                    <h3 class="text-sm font-bold text-slate-800 uppercase tracking-wider heading-font">Daftar Siswa yang Sudah Hadir</h3>
                    ...
                    @forelse($session->attendances()->where('status', 'HADIR')->with('student')->get() as $att)
                        <div class="p-3 sm:p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 flex items-center justify-between gap-2.5 sm:gap-3 text-xs hover:bg-slate-100/70 transition">
                            <div class="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                                <div class="w-8 h-8 rounded-xl bg-emerald-600 text-white font-black text-xs flex items-center justify-center shrink-0 shadow-xs">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
                                    </svg>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <p class="font-bold text-slate-900 text-xs sm:text-sm leading-snug break-words">{{ $att->student->name }}</p>
                                    <p class="text-[10px] sm:text-[11px] text-slate-500 mt-0.5">NISN: <span class="mono-font">{{ $att->student->identity_number }}</span></p>
                                </div>
                            </div>
                            <span class="shrink-0 px-2 sm:px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-bold bg-emerald-100 text-emerald-800 mono-font whitespace-nowrap">
                    ...
                    data.verified_students.forEach(st => {
                        ...
                        item.innerHTML = `
                            <div class="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="font-bold text-slate-900 text-xs sm:text-sm leading-snug break-words">${escapeHtml(st.name)}</p>
