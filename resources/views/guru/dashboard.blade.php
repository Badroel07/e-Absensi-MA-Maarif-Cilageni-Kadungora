...
<div class="space-y-6 font-sans-card">
    ...
    <!-- 1. PROFILE SECTION (Card Profil Guru - Varian 2 Emerald Gradient Soft Glass) -->
    <section class="relative overflow-hidden text-white rounded-2xl p-5 shadow-lg shadow-emerald-950/20 border border-emerald-500/30" 
        ...
        <div class="absolute -bottom-8 -left-8 w-28 h-28 bg-emerald-400/10 rounded-full blur-xl pointer-events-none"></div>

        <div class="relative z-10 space-y-4">
            <!-- Top Row -->
            <div class="flex items-start justify-between gap-3">
                <div class="flex items-center gap-3">
                    <!-- Avatar with Solid White Background for striking contrast -->
                    <div class="w-12 h-12 rounded-2xl bg-white text-[#047857] font-extrabold text-xl flex items-center justify-center shadow-md shrink-0 overflow-hidden">
                    ...
                    <div class="min-w-0">
                        <h3 class="text-base font-bold text-white tracking-tight leading-tight truncate mt-0.5">{{ $teacher->name }}</h3>
                    </div>
                </div>
            </div>

            <!-- Glass Inner Pill Bar (Location & Live Clock) -->
                ...
                <div class="flex items-center justify-end gap-2 px-1 border-l border-white/10 text-right">
                    <div class="min-w-0">
                        ...
                        <div class="flex items-baseline justify-end gap-1">
                            <span class="font-bold text-white tracking-wider text-xs liveClockTicker">{{ now()->format('H:i:s') }}</span>
        ...
        <details class="group bg-white rounded-2xl border border-slate-200/70 shadow-sm overflow-hidden transition-all duration-200">
            ...
            <div class="px-4 pb-4 pt-2 border-t border-slate-100 text-xs text-slate-600 grid grid-cols-1 sm:grid-cols-2 gap-2.5 bg-slate-50/40">
                ...
                <div class="p-2.5 bg-white rounded-xl border border-slate-100 flex items-start gap-2.5">
                    ...
                    <div class="leading-relaxed"><strong class="text-slate-900">Absen Sesi:</strong> Kehadiran sesi mengajar tercatat otomatis saat sesi dibuka.</div>
                </div>
            </div>
        </details>
    </section>
            ...
            <div class="space-y-3.5">
                @foreach($schedules as $index => $sch)
                    @php
                        ...
                        $teacherAtt = $sch->todayTeacherAttendance;
                        ...
                        $sessionAttLabels = ['IZIN' => 'Izin', 'SAKIT' => 'Sakit', 'DINAS_LUAR' => 'Dinas Luar', 'ALPA' => 'Alpa'];
                    @endphp
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <div class="space-y-1 min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    @if($teacherAtt)
                                        @if($teacherAtt->status === 'HADIR')
                                            <span class="px-2 py-0.5 rounded-md text-[10px] font-bold {{ $teacherAtt->isLate() ? 'bg-amber-100 text-amber-800 border border-amber-200' : 'bg-emerald-100 text-emerald-800 border border-emerald-200' }} whitespace-nowrap">
                                                Absen {{ $teacherAtt->attended_at?->format('H:i') }} — Hadir{{ $teacherAtt->isLate() ? ' (Terlambat)' : '' }}
                                            </span>
                                        @else
                                            <span class="px-2 py-0.5 rounded-md text-[10px] font-bold bg-rose-100 text-rose-700 border border-rose-200 whitespace-nowrap">
                                                {{ $sessionAttLabels[$teacherAtt->status] ?? $teacherAtt->status }}
                                            </span>
                                        @endif
                                    @endif
                                </div>
                            ...
                            @if(!$session)
                                ...
                                @else
                                    <form action="{{ route('guru.session.open', $sch) }}" method="POST" data-loading-form class="openSessionForm w-full flex gap-2">
                                        @csrf
                                        <input type="hidden" name="latitude" class="js-geo-lat">
                                        <input type="hidden" name="longitude" class="js-geo-lng">
    ...
    <!-- 5. PRESENSI KEDATANGAN & SESI MENGAJAR -->
    @php
        $sessionAtts = $schedules->map(fn ($s) => $s->todayTeacherAttendance);
        $saHadir = $sessionAtts->where('status', 'HADIR')->count();
        $saTerlambat = $sessionAtts->filter(fn ($att) => $att && $att->isLate())->count();
        $saBelum = $schedules->count() - $saHadir;
    @endphp
    <section class="space-y-3.5 pt-2" data-purpose="teacher-attendance">
        <div class="flex items-center justify-between mb-3">
            <h3 class="text-xs font-bold uppercase tracking-wider heading-font text-slate-900">Presensi Kedatangan & Sesi Mengajar</h3>
        </div>
        ...
        <article class="bg-white rounded-2xl border border-slate-200/70 shadow-sm p-4 space-y-4 hover:border-slate-300/80 transition-colors">
            <div class="flex items-center justify-between gap-2 pb-3 border-b border-slate-100">
                <h4 class="text-base font-bold text-slate-900 tracking-tight heading-font">Kedatangan Hari Ini</h4>
                <p class="flex items-center gap-1.5 text-xs font-medium whitespace-nowrap shrink-0 {{ $hasCheckedIn ? 'text-emerald-800' : 'text-slate-600' }}">
                    <i data-lucide="{{ $hasCheckedIn ? 'check-circle-2' : 'clock' }}" class="w-3.5 h-3.5 {{ $hasCheckedIn ? 'text-emerald-700' : 'text-slate-400' }}"></i>
                    {{ $hasCheckedIn ? 'Sudah Presensi' : 'Belum Presensi' }}
                </p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2.5 items-stretch">
                ...
                <!-- Sesi Mengajar -->
                <div class="bg-slate-50/80 border border-slate-100 rounded-xl p-3 flex flex-col justify-between min-h-[82px]">
                    <span class="text-xs text-slate-500 font-medium">Absen Sesi Mengajar</span>
                    <div class="mt-1 space-y-1 text-[11px] leading-tight">
                        <div class="flex items-center justify-between gap-1">
                            <span class="text-slate-500">Hadir</span>
                            <span class="font-bold text-emerald-700 mono-font">{{ $saHadir }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-1">
                            <span class="text-slate-500">↳ Terlambat</span>
                            <span class="font-bold text-amber-700 mono-font">{{ $saTerlambat }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-1">
                            <span class="text-slate-500">Belum Absen</span>
                            <span class="font-bold text-slate-600 mono-font">{{ max(0, $saBelum) }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
(function() {
    ...
    function handleFormSubmit(e) {
        if (!isWithinGeofence || !currentCoords.lat) {
            e.preventDefault();
            alert('Buka sesi terkunci: Posisi GPS belum terverifikasi berada di dalam lingkungan madrasah.');
            return;
        }
        e.currentTarget.querySelectorAll('.js-geo-lat').forEach(el => { el.value = currentCoords.lat; });
        e.currentTarget.querySelectorAll('.js-geo-lng').forEach(el => { el.value = currentCoords.lng; });
    }
