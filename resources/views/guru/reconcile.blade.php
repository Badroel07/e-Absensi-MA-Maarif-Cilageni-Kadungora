...
@section('content')
<div class="space-y-4 max-w-xl mx-auto" x-data="{
    activeTab: 'all',
    searchQuery: '',
    selectedStatuses: {},
    init() {
    ...
    <div class="pt-1 pb-0.5 flex items-center gap-3">
        ...
        <div class="flex-1 min-w-0">
            <h2 class="text-lg font-bold text-slate-900 tracking-tight leading-tight heading-font">Konfirmasi Kehadiran Siswa</h2>
            <p class="text-xs text-slate-500 mt-0.5">Kelola dan finalisasi status kehadiran presensi kelas</p>
        </div>
    </div>

    <section class="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-sm space-y-4" data-purpose="session-meta">
        ...
        <div class="flex items-start gap-3">
            <div class="flex-1 min-w-0">
                ...
                <h3 class="text-lg font-bold text-slate-900 tracking-tight leading-snug heading-font">
                    {{ $session->schedule->subject->name }}
                </h3>
                <div class="flex items-center text-xs text-slate-500 font-medium mt-1.5 gap-1">
        ...
        <!-- Metric Badges Row -->
        <div class="grid grid-cols-3 gap-2 pt-3.5 border-t border-slate-100 text-center">
            <div class="bg-slate-50 border border-slate-200/60 rounded-xl p-2.5">
                ...
                <span class="text-base font-bold text-slate-800 mt-1 block leading-tight mono-font heading-font">{{ $totalCount }}</span>
            </div>
            <div class="bg-emerald-50/70 border border-emerald-100 rounded-xl p-2.5">
                ...
                <span class="text-base font-bold text-emerald-700 mt-1 block leading-tight mono-font heading-font">{{ $hadirCount }}</span>
            </div>
            <div class="bg-rose-50/70 border border-rose-100 rounded-xl p-2.5">
                ...
                <span class="text-base font-bold text-rose-600 mt-1 block leading-tight mono-font heading-font">{{ $unverifiedCount }}</span>
            </div>
        </div>
    </section>
    <!-- END: SessionHeaderMeta -->
    ...
    @if($isLocked ?? false)
        <section class="bg-gradient-to-br from-rose-50 via-rose-50/60 to-white border border-rose-200 rounded-2xl p-4 shadow-soft" data-purpose="status-alert-banner">
            <div class="flex items-start gap-3">
                <div class="w-9 h-9 rounded-xl bg-rose-600 text-white flex items-center justify-center shrink-0 shadow-sm shadow-rose-200 mt-0.5">
                ...
                <div class="flex-1 text-xs">
                    <h3 class="text-sm font-bold text-rose-950 leading-snug heading-font">
    ...
    <section class="space-y-2.5" data-purpose="filtering-and-search">
        ...
        <div class="relative">
            ...
            <input x-model="searchQuery" class="w-full bg-white text-slate-800 text-xs rounded-xl pl-9 pr-4 py-2.5 border border-slate-200/90 focus:outline-none focus:ring-2 focus:ring-emerald-600/20 focus:border-emerald-600 transition placeholder:text-slate-400 shadow-sm" placeholder="Cari nama atau NISN..." type="search">
        </div>
    </section>
    <!-- END: FilterAndSearch -->
        ...
        <section class="space-y-3 pt-1" data-purpose="students-attendance-list">
            @foreach($students as $st)
                ...
                <article x-show="(activeTab === 'all' || (activeTab === 'verified' && {{ $isVerifiedHadir ? 'true' : 'false' }}) || (activeTab === 'unverified' && {{ $isVerifiedHadir ? 'false' : 'true' }})) && ('{{ strtolower($st->name) }} {{ $st->identity_number }}'.includes(searchQuery.toLowerCase().trim()))"
                         class="bg-white rounded-2xl p-3.5 border border-slate-200/90 shadow-soft space-y-3">
                    ...
                    <div class="relative">
                        ...
                        <input class="w-full text-xs pl-10 pr-3.5 py-2.5 bg-slate-50/70 rounded-xl border border-slate-200 focus:bg-white focus:outline-none focus:ring-1 focus:ring-emerald-600 focus:border-emerald-600 text-slate-700 placeholder:text-slate-400 transition {{ ($isLocked ?? false) ? 'cursor-not-allowed opacity-75' : '' }}"
                    ...
                    <!-- Status Action Pills (no outer border on selection - matches reference code exactly) -->
                    <div class="grid grid-cols-4 gap-1.5 pt-0.5">
                        ...
                        <button type="button" @click="setStatus('{{ $st->id }}', 'HADIR')"
                                ...
                                class="inline-flex items-center justify-center gap-1 py-1.5 rounded-lg text-xs transition-all cursor-pointer select-none {{ ($isLocked ?? false) ? 'cursor-not-allowed opacity-80' : '' }}">
                        ...
                        <button type="button" @click="setStatus('{{ $st->id }}', 'IZIN')"
                                :class="selectedStatuses['{{ $st->id }}'] === 'IZIN'
                                    ...
                                    : 'text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 font-semibold'"
                                class="inline-flex items-center justify-center gap-1 py-1.5 rounded-lg text-xs transition-all cursor-pointer select-none {{ ($isLocked ?? false) ? 'cursor-not-allowed opacity-80' : '' }}">
                            <span :class="selectedStatuses['{{ $st->id }}'] === 'IZIN' ? 'text-white' : 'text-blue-500'">📄</span>
                            <span>Izin</span>
                        </button>
                        ...
                        <button type="button" @click="setStatus('{{ $st->id }}', 'SAKIT')"
                                :class="selectedStatuses['{{ $st->id }}'] === 'SAKIT'
                                    ? 'bg-sky-500 text-white font-bold border border-sky-500 shadow-sm'
                                    : 'text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 font-semibold'"
                                class="inline-flex items-center justify-center gap-1 py-1.5 rounded-lg text-xs transition-all cursor-pointer select-none {{ ($isLocked ?? false) ? 'cursor-not-allowed opacity-80' : '' }}">
